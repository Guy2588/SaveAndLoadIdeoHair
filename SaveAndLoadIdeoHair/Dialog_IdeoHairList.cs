...
namespace SaveAndLoadIdeoHair
{
    ...
    public abstract class Dialog_IdeoHairList : Dialog_FileList
    {
        ...
        public MethodInfo GenFilePaths_FolderUnderSaveData = typeof(GenFilePaths).GetMethod("FolderUnderSaveData", BindingFlags.NonPublic | BindingFlags.Instance, null, CallingConventions.Any, new Type[] { typeof(string) }, null);
		...
		public string SavedColonistsFolderPath
		{
			get
			{
				try
		...
		public IEnumerable<FileInfo> AllCustomIdeoHairFiles {
			get {
				...
				return from f in directoryInfo.GetFiles()
					   where f.Extension == ".rid" //this probably should be changed (maybe .idh (for IDeoligion Hair)), gonna have to mess around with the saver + loader
